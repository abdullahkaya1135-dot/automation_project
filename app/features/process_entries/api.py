...
from .payloads import combined_entry_payload
...
def list_entries(
    ...
    if sort not in {"process", "recent"}:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="sort sunlardan biri olmalidir: process, recent",
        )

    settings = settings_from_request(request)
    if sort == "recent":
        order_by: tuple[Any, ...] = (
            func.coalesce(Entry.submitted_at, Entry.created_at).desc(),
            Entry.id.desc(),
        )
    else:
        order_by = (
...
def retry_sync(
    ...
    payload = retry_unsynced_entries(
        settings_from_request(request),
        limit=limit,
        process_date=process_date,
    )
    payload["database_backed"] = True
    return payload
